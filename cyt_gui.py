...
cyt_dir = os.path.expanduser('~/cyt')

def write_slogan():
    print("Checking Status")
    subprocess.call(["x-terminal-emulator", "-e", f'{cyt_dir}/monitor.sh'])

def func_delete_ignore():
    print("Deleting Ignore Lists")
    subprocess.call(["x-terminal-emulator", "-e", f'{cyt_dir}/delete_ignore_lists.sh'])

def func_create_ignore():
    print("Creating Ignore Lists")
    subprocess.call(["x-terminal-emulator", "-e", "python3", f'{cyt_dir}/create_ignore_list.py'])

def func_run_cyt():
    print("Running CYT")
    subprocess.call(["x-terminal-emulator", "-e", f'{cyt_dir}/chasing_your_tail.sh'])
